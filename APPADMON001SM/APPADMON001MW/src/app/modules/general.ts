...
export class General {
  ...
  constructor(
    ...
    setDate(days: number = 0, months : number = 0): string {
      ...
      currencyToMXN(MXN: number): string {
        const currencyPipe = new CurrencyPipe('en-US');
        return currencyPipe.transform(MXN, '', 'symbol', '1.2-2');
      }
    
      currencyToDLLS(DLLS: number, ): string {
        const currencyPipe = new CurrencyPipe('en-US');
      ...
      generateReportURL(reportName : string, database : string, folder : string , URLParameters : any[], showParams: boolean = true, openAs : string = '_blank', applyZona : boolean = false) : string{
        let Zona = ""
        if(applyZona){
        ...
        'http://datos.cecso.com.mx/ServerRS&usuarioSRS=reportes&pswSRS=R3portes16&NombreSRS='+
         reportName + '&rutaSRS=/' + database + Zona+
        '/'+folder+'/'+reportName+'&parametrosSRS=';
    
        //const URLParameters = [];
       
        /*  URLParameters.push('FechaInicio?' + this.filtros.fechaInicio);
          URLParameters.push('FechaFin?' +  this.filtros.fechaFin);*/
        
        
        let URLExtra : string = "";
        if(showParams){
          URLExtra = "&val=ITEwISE5IYm6EzYVbEfJq0iuWExA"
        }
        else{
          URLExtra = "&val=ITEwISE5IYm6EzYVbD1SSzdhl50s"
        }
         
        const URLReporte = URLBase + (URLParameters.length > 0 ? URLParameters.join(',') : 'null') + URLExtra;
        //window.open(URLReporte, openAs).focus();
